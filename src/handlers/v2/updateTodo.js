...
module.exports.handler = async (event) => {
  ...
  try {
    ...
    if (body.environment !== undefined) {
      ...
      attributeValues[':environment'] = body.environment;
    }

    if (body.isSeedData !== undefined) {
      updates.push('isSeedData = :isSeedData');
      attributeValues[':isSeedData'] = body.isSeedData;
    }

    // Prevent empty update requests
    if (updates.length === 0) {
      return response(400, {
        message: 'At least one field (title, description, completed, environment, isSeedData) must be provided',
      });
    }
